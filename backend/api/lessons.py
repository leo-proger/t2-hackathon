...
async def get_filter(data: LessonFilterSchema, request: Request, session: SessionDep):
    """
    ...
    query = select(LessonModel)
    if data.date:
        query = query.filter(LessonModel.date == data.date)
    if data.subgroup:
...
@router.get('/personal/get_day')
async def personal_get_day(request: Request, session: SessionDep):
    """
    Возвращает статус "ok" и все пары соответствующие настройкам фильтра.
